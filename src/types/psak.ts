// ========================================
// 财务报表 / 尽调清单 / 思维链 — 类型定义
// 列报与科目引用字段名沿用历史 psakCode 等，实际展示为 CAS / 企业会计准则
// ========================================
...
export const STATEMENT_LABELS: Record<StatementType, { zh: string; id: string }> = {
  balance_sheet: { zh: '资产负债表', id: 'BS' },
  income_statement: { zh: '利润表', id: 'P&L' },
  cash_flow: { zh: '现金流量表', id: 'CF' },
};

// 三表行项
export interface FinancialLineItem {
  id: string;
  label: string;           // 次行：准则简称 / CAS 等（可空）
  labelZh: string;         // 主行：中文科目名
  currentYear: number;     // 当期金额（万元 · 人民币）
  priorYear: number;       // 上期金额
  psakCode: string;        // 历史字段名；展示为 CAS 等准则引用
...
export interface CrossValidationRule {
  ...
  psakRef: string;         // 准则或内部规则引用（字段名历史遗留）
}
...
export const CHECKLIST_CATEGORY_LABELS: Record<ChecklistCategory, { zh: string; id: string }> = {
  financial: { zh: '财务资料', id: 'Financial' },
  legal: { zh: '法律文件', id: 'Legal' },
  operational: { zh: '经营资料', id: 'Operations' },
  collateral: { zh: '担保物', id: 'Collateral' },
  compliance: { zh: '合规文件', id: 'Compliance' },
};
...
export interface ChecklistItem {
  ...
  documentNameId: string;     // 英文 / 内部编码别名
