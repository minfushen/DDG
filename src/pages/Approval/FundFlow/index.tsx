...
const nodeTypeConfig: Record<FundFlowNodeType, { label: string; gradient: GradientKey; icon: React.ElementType }> = {
  borrower: { label: '借款企业', gradient: 'blue', icon: Building2 },
  supplier: { label: '供应商', gradient: 'green', icon: Building2 },
  third_party: { label: '第三方', gradient: 'blue', icon: Building2 },
  individual: { label: '个人账户', gradient: 'blue', icon: User },
  real_estate: { label: '房地产', gradient: 'red', icon: Home },
  stock: { label: '证券账户', gradient: 'amber', icon: TrendingUp },
  shell: { label: '空壳公司', gradient: 'red', icon: AlertCircle },
};
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-white rounded-2xl shadow-lg shadow-gray-200/50 p-6 animate-fade-in-up">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 bg-[var(--risk-medium)] rounded-full" />
                ...
                <div key={node.id} className="p-3 bg-[var(--risk-high-bg)] rounded-xl border border-[var(--risk-high-bg)] animate-fade-in-up" style={{ animationDelay: `${index * 50}ms` }}>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {node.riskLabels?.map((label, i) => (
                      <span key={i} className="px-2 py-0.5 bg-[var(--risk-high-bg)] text-[var(--risk-high-text)] rounded-lg text-xs">{label}</span>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-3">
              {suspiciousNodes.map((node, index) => (
                <div key={node.id} className="p-3 bg-[var(--risk-medium-bg)] rounded-xl border border-[var(--risk-medium-bg)] animate-fade-in-up" style={{ animationDelay: `${index * 50}ms` }}>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {node.riskLabels?.map((label, i) => (
                      <span key={i} className="px-2 py-0.5 bg-[var(--risk-medium-bg)] text-[var(--risk-medium-text)] rounded-lg text-xs">{label}</span>
                    ))}
                  </div>
                </div>
              ))}
                  ...
                  <div key={edge.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg animate-fade-in-up" style={{ animationDelay: `${index * 50}ms` }}>
                    <div className="flex items-center gap-2">
                      {edge.invoiceMatched ? (
                        ...
                        <FileX className="w-4 h-4 text-[var(--risk-high)]" />
                      )}
