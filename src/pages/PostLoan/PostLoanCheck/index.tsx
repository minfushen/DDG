        ...
        <div className="space-y-8">
          ...
          <section className="section-shell rounded-[12px]">
            <SectionHeader
              ...
              className="px-6 pt-6"
              actions={
                <button className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-white hover:bg-primary-deep transition-colors">
            ...
            <div className="px-6 pb-6">
              {pendingChecks.filter((c) => c.status !== 'overdue').length > 0 ? (
                ...
                <div className="p-6 bg-[var(--color-bg-layout)] rounded-lg text-center">
                  <p className="text-[var(--color-text-tertiary)]">暂无待检查任务</p>
                </div>
              )}
            </div>
          </section>
          ...
          <section className="section-shell rounded-[12px]">
            ...
            <div className="px-6 pb-6">
              {inProgressChecks.length > 0 ? (
                ...
                <div className="p-6 bg-[var(--color-bg-layout)] rounded-lg text-center">
                  <p className="text-[var(--color-text-tertiary)]">暂无进行中的检查</p>
                </div>
              )}
            </div>
          </section>
        ...
        <section className="section-shell rounded-[12px]">
          ...
          <div className="px-6 pb-6">
            {completedChecks.length > 0 ? (
              ...
              <div className="p-6 bg-[var(--color-bg-layout)] rounded-lg text-center">
                <p className="text-[var(--color-text-tertiary)]">暂无已完成的检查</p>
              </div>
            )}
          </div>
        </section>
    ...
    >
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 rounded-lg bg-primary-bg flex items-center justify-center shrink-0">
          <ClipboardCheck className="w-4 h-4 text-primary-deep" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs text-[var(--color-text-tertiary)]">{typeC.label}</span>
            <StatusBadge status={check.status} config={checkStatusConfig} />
          </div>
          <p className="text-sm font-medium text-[var(--color-text-primary)] truncate">{check.enterpriseName}</p>
          ...
          {check.status === 'in_progress' && (
            <div className="mt-2">
              ...
              <div className="h-1.5 bg-[var(--color-progress-trail)] rounded-full overflow-hidden">
                <div
        ...
        <ChevronRight className="w-4 h-4 text-[var(--color-text-quaternary)] shrink-0" />
      </div>
    </button>
  );
}
...
function CompletedCheckCard({ check, onClick }: { check: PostLoanCheck; index: number; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left p-3 rounded-lg bg-[var(--color-bg-layout)] border border-[var(--color-card-border)] hover:border-[var(--color-primary-border)] transition-all"
    >
