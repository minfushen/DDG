...
LLM_NARRATIVE_TIMEOUT_SECONDS = 60
...
def _get_primary_narrative_llm() -> ChatOpenAI:
    """Use a smaller response budget for short financial prose."""
    return ChatOpenAI(
        ...
        temperature=0,
        max_tokens=3072,
        timeout=min(max(settings.LLM_TIMEOUT_SECONDS, LLM_NARRATIVE_TIMEOUT_SECONDS), 90),
        max_retries=1,
    )
...
def _fallback_diagnostics(
    ...
    return {
        ...
        "diagnostics": [
            {
                ...
                "driver": "收入增长质量需结合主营构成、订单连续性、费用结构、非经常性损益和行业景气度复核；若利润修复弱于收入扩张，需警惕规模增长未能转化为主营造血能力。",
            ...
            },
            {
                ...
                "driver": "若应收增速高于收入增速或经营现金流弱于利润表现，需关注信用政策放宽、客户集中回款滞后和收入确认质量。",
                "risk_level": "中风险",
            ...
            },
            {
                ...
                "driver": "杠杆变化和流动比率下降需结合短期借款、应付账款、票据融资、或有负债及未使用授信额度判断，报表流动性不等同于可即时偿债现金。",
        ...
        "manual_review_items": ["审计报告及附注", "银行流水", "纳税资料", "主要合同", first_risk],
    }
...
def _fallback_summary(
    ...
    return _render_diagnostics(
        _fallback_diagnostics(enterprise_name, key_metrics, risk_summary, recommendation, risk_rating, risk_score, data_boundary, key_metric_series)
    )
...
def _number_warnings(
    ...
    allowed = _allowed_numbers(key_metrics, years, key_metric_series, risk_score)
    warnings = []
    text = "\n".join(summary)
    ...
    for number in re.findall(r"\d+(?:\.\d+)?", text):
        if number not in allowed:
            # Ignore ordinary phrase fragments such as 12个月 in review guidance.
            if number in {"1", "2", "3", "4", "5", "6", "12", "70", "80", "100"}:
                continue
...
def _quality_warnings(
    ...
    warnings.extend(_number_warnings(summary, key_metrics, years, key_metric_series, risk_score))
    return warnings
...
def _prompt(
    ...
    allowed_numbers = sorted(_allowed_numbers(key_metrics, years, key_metric_series, risk_score))
    knowledge_context = knowledge_context or {}
    triggered_rules = knowledge_context.get("triggered_rules") or []
...
6. 当应收增速或应收/营收偏高时，必须关联经营现金流、信用政策、期后回款和坏账计提。
7. 当资产负债率上升或流动比率下降时，必须关联短债、应付账款/票据、或有负债和未来12个月偿债压力。
8. 只使用输入数据，不编造数字；不得引入行业均值、行业平均、同业水平等未提供数字；除段落编号和“12个月”外，输出中只能出现这些数字：{', '.join(allowed_numbers)}。
9. 公告/搜索线索可用于解释“可能原因”和“核查方向”，但不得把其中数字写入结论，除非该数字也出现在最新指标或近三年指标序列中。
10. 缺失原因、行业基准或同业数据时用“[需补充：...]”，不要自行估算。
11. 禁止：{', '.join(BANNED_TERMS)}。
...
def _parse_candidate(
    ...
    if diagnostics:
        summary = _render_diagnostics(diagnostics)
    else:
    ...
    warnings = _diagnostic_warnings(diagnostics) + _quality_warnings(summary, key_metrics, years, key_metric_series, risk_score)
...
def build_financial_narrative(
    ...
    fallback = _render_diagnostics(fallback_diagnostics)
    started_at = time.monotonic()
    try:
        candidate = _invoke_llm_race_with_timeout(
            _prompt(enterprise_name, years, key_metrics, key_metric_series, risk_summary, recommendation, risk_rating, risk_score, data_boundary, knowledge_context, public_context),
            fallback,
            fallback_diagnostics,
            key_metrics,
            years,
