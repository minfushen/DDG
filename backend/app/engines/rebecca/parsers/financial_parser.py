"""财报文件解析器（P1.1：非上市公司 PDF 路由到 FinancialPdfPipeline）。

本模块从原 ``rebecca/parsers.py`` 迁入包内，修复 ``parsers/__init__.py`` 空文件
导致 ``from app.engines.rebecca.parsers import FinancialParser`` 失败的问题，并在
``_parse_pdf`` 中实现 P1.1：扫描件 PDF 优先路由到四阶段管道，降级 pdfplumber。
"""
import pandas as pd
...
class FinancialParser:
    ...
    def _parse_pdf(self, file_path: Path) -> FinancialData:
        ...
        非上市公司扫描件 PDF 优先路由到 ``FinancialPdfPipeline``（四阶段：MinerU 版面分析
        → MiniCPM-V 只抄不算 → AccountingValidator 勾稽 → VisionFallback），拿到结构化
        三大表 + 解析质量信号（auto_judgment_rate / needs_human_review / issues）。
        pipeline 不可用或失败时降级 pdfplumber，metadata 标记 fallback。
        """
        is_scanned = self._detect_scanned_pdf(file_path)

        if is_scanned:
            pipeline_data = self._parse_pdf_via_pipeline(file_path)
            if pipeline_data is not None:
                return pipeline_data
            # pipeline 失败则降级 pdfplumber

        data = self._parse_pdf_via_pdfplumber(file_path)
        if is_scanned:
            data.metadata["parser"] = "pdfplumber_fallback"
            data.metadata["pipeline_failed"] = True
        else:
            data.metadata["parser"] = "pdfplumber"
        return data

    def _detect_scanned_pdf(self, file_path: Path) -> bool:
        """检测 PDF 是否为扫描件（无文字层或文字极少）。

        抽样前 5 页，累计可见文字 < 200 字符则判定为扫描件。
        检测本身失败时不强制走 pipeline（返回 False），避免误伤。
        """
        try:
            import pdfplumber
    ...
    def _parse_pdf_via_pipeline(self, file_path: Path) -> Optional[FinancialData]:
        """通过 FinancialPdfPipeline 解析，返回 FinancialData 或 None（失败/不可用）。"""
        try:
            from .financial_pdf_pipeline import (
                parse_annual_report_pdf,
                pipeline_statements_to_standard_dataframes,
            )

            pdf_bytes = file_path.read_bytes()
            enterprise_name = file_path.stem
            ...
            standard = pipeline_statements_to_standard_dataframes(result.statements)
            return FinancialData(
                income_statement=standard["income_statement"],
                balance_sheet=standard["balance_sheet"],
                cash_flow=standard["cash_flow"],
                metadata={
                    "source_file": str(file_path),
                    "format": "pdf",
                    "parser": "financial_pdf_pipeline",
                    "auto_judgment_rate": result.auto_judgment_rate,
                    "needs_human_review": result.needs_human_review,
                    "issues": list(result.issues),
                    "main_table_coverage": result.main_table_coverage,
                    "rule_family_coverage": result.rule_family_coverage,
                    "digital_reach_rate": result.digital_reach_rate,
                },
            )
        except Exception:
            return None

    def _parse_pdf_via_pdfplumber(self, file_path: Path) -> FinancialData:
        """pdfplumber 降级解析（原 _parse_pdf 逻辑）。"""
        try:
            import pdfplumber
